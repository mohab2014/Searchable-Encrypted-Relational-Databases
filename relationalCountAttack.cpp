...
//Implementation of Abdelraheem et al. Relational-Count attack published at DPM 2017 workshop co-located with ESORICS 2017
#include <cstdlib>
...
int main(int argc, char* argv[]) 
{
  ...
  if(argc != 10)
  {
       ...
       std::cout<<"./relationCountAttack allKeywordsfile obsFullMatFile TotalKeywords ResultSizeFile obsQueryMat QuerySize NO_OF_RECORDS cheatsheet folderName\n";
       exit(1);
  }
  ...
  for( std::string line; getline(resultFileSize, line ); )
  {
        ...
        long result = atoi(tokens[2].c_str());
  ...
  do
  {
      ...
      for(int i = 0;i < unresolvedQueryNoList.size();i++)
      {
          ...
          for(int j = 0;j < S.size();j++)
          {
                ...
                string possibleKeyword = unresolvedCandidateList[queryNoCT1][j];
                string possibleKeyword_attributeName = possibleKeyword.substr(0,possibleKeyword.find_first_of(":"));
          
                for(int k = 0;k < resolvedQueriesMap[possibleKeyword_attributeName].size();k++)
                {
                         int recoveredQueryNo  = resolvedQueriesMap[possibleKeyword_attributeName][k];
                     
                  
                         //cout<<"recoveredQueryNo = "<<recoveredQueryNo<<", possibleKeyword_attributeName = "<<possibleKeyword_attributeName<<endl;  
              
                         if(CTmatrix[queryNoCT1][recoveredQueryNo] != 0)
                         {
                            //removeS[j] = true;
                            //cout<<" S["<<j<<"] = "<<S[j]<<", keyword = "<<unresolvedCandidateList[queryNoCT1][j]<<", Action: Remove"<<", Co-occurrence = "<<CTmatrix[queryNoCT1][recoveredQueryNo]<<endl;
                            removeS[j] = true;
                            break;
                         }
                         //else                     
                           //cout<<" S["<<j<<"] = "<<S[j]<<", keyword = "<<unresolvedCandidateList[queryNoCT1][j]<<", Action: Keep"<<", Co-occurrence = "<<CTmatrix[queryNoCT1][recoveredQueryNo]<<endl;
